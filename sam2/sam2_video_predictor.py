...
class SAM2VideoPredictor(SAM2Base):
    ...
    #             if frame_idx in obj_output_dict["cond_frame_outputs"]:
    #                 # Conditioning frame (user-provided mask/clicks) — always use as-is
    #                 storage_key = "cond_frame_outputs"
    #                 current_out = obj_output_dict[storage_key][frame_idx]
    #                 device = inference_state["device"]
    #                 pred_masks = current_out["pred_masks"].to(device, non_blocking=True)
    ...
    #                 # ← NEW: already computed on a previous propagation pass — reuse,
    #                 # do NOT re-run inference. This prevents established objects from
    #                 # being overwritten every time a new object is admitted mid-video.
    #                 storage_key = "non_cond_frame_outputs"
    #                 current_out = obj_output_dict[storage_key][frame_idx]
    #                 device = inference_state["device"]
    #                 pred_masks = current_out["pred_masks"].to(device, non_blocking=True)
    #             else:
    #                 # Genuinely unseen frame for this object — run inference
    #                 storage_key = "non_cond_frame_outputs"
    #                 current_out, pred_masks = self._run_single_frame_inference(
    #                     inference_state=inference_state,
    #                     output_dict=obj_output_dict,
